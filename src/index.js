...
#!desc=111111111111111111111111111111111111111111111111111111111111111111。适用于 Shadowrocket / Surge / Egern。
#!homepage=${origin}
...
function stoverride(origin) {
  return String.raw`name: iOS Location Spoofer (Stateless)
desc: "11111111111111111111111111111。iOS Location Spoofer 无状态版 (Stash)"
homepage: ${origin}
...
function lnplugin(origin) {
  return String.raw`#!name=iOS Location Spoofer (Stateless)
#!desc=11111111111111111111111111111111。Loon 插件。
#!homepage=${origin}
...
function qxsnippet(origin) {
  return String.raw`#!name=iOS Location Spoofer (Stateless)
#!desc=1111111111111111111111111111111。Quantumult X 用「重写(rewrite)引用」(非模块/插件)。MITM 主机名需手动加进 QX 设置 → MITM。
#!homepage=${origin}
...
app.post("/tg", async (c) => {
  ...
  if (token && chatId && (cmd === "/link" || cmd === "/links" || cmd === "/start")) {
    ...
    const reply =
      "📍 iOS 虚拟定位 · 选点主页\n" + origin + "/\n\n" +
      "⚠️ 111111111111111111111111111111111111111111111";
